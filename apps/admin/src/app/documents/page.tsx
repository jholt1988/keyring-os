...
import { FolderOpen, Upload, Download, Search, RefreshCw, FileText } from 'lucide-react';
        ...
        <SectionCard title="Documents">
          ...
          <div className="mb-6 flex items-center gap-2 text-sm text-[#94A3B8]">
            <button 
              ...
              className={`transition-colors hover:text-[#F8FAFC] ${!activeCategory ? 'font-medium text-[#F8FAFC]' : ''}`}
            >
              Root
            </button>
            {activeCategory && (
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              {/* Folders View (Only show at root level and when not searching) */}
              {!activeCategory && !search && categories.length > 0 && (
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
                  {categories.map(cat => {
                    const count = (docs as any[]).filter(d => (d.category?.toUpperCase() ?? 'OTHER') === cat).length;
                    return (
                      <button 
                        key={cat} 
                        onClick={() => setActiveCategory(cat)}
                        className="group flex flex-col items-center justify-center rounded-xl border border-[#1E3350] bg-[#0F1B31] p-4 transition-all hover:border-[#3B82F6] hover:bg-[#1E3350]/30"
                      >
                  ...
                  {filtered.length === 0 ? (
                    <div className="py-12 text-center">
                      ...
                      <p className="text-sm text-[#94A3B8]">
                        {search ? 'No documents match your filter.' : 'This folder is empty.'}
                      </p>
                      {!search && (
                        <Button size="sm" className="mt-4" onClick={() => { setUploadCategory(activeCategory || 'OTHER'); setUploadOpen(true); }}>
                          <Upload size={13} /> Upload document here
                        </Button>
                      )}
                    </div>
                  ) : (
          ...
          {!isLoading && (activeCategory || search) && filtered.length > 0 && (
            <p className="mt-4 text-xs text-[#94A3B8]">{filtered.length} documents</p>
          )}
        </SectionCard>
      </WorkspaceShell>
